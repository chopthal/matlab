function analyte = ParsingPath(parentPath, eventTime)
...
%     analyte(analyteNo).DefaultFittingVariable.OneToOneStandard.Type = {'Local'; 'Local'; 'Local'; 'Local'};
    ...
    kt = ones(1, size(analyte(analyteNo).Concentration, 1)) * 10^8;
    targetIdx = 3;
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer = struct;
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer =...
        analyte(analyteNo).DefaultFittingVariable.OneToOneStandard;    
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.Name = InsertElementToCell(...
       analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.Name, targetIdx, 'kt');
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.Type = InsertElementToCell(...
       analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.Type, targetIdx, 'Global');
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.InitialValue = InsertElementToCell(...
       analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.InitialValue, targetIdx, kt);
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.UpperBound = InsertElementToCell(...
       analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.UpperBound, targetIdx, kt * boundScale);   
    analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.LowerBound = InsertElementToCell(...
       analyte(analyteNo).DefaultFittingVariable.OneToOneMassTransfer.LowerBound, targetIdx, kt / boundScale);
    
   analyte(analyteNo).FittingVariable = analyte(analyteNo).DefaultFittingVariable;
end
