function h = montageWithoutImshow(varargin)
...
%   MONTAGE displays the images horizontally across columns.
%
...
%   cell array IMAGELIST. Elements of the cell array are either numeric
...
%   Parameters
%   ----------
%   'Size'            A 2-element vector, [NROWS NCOLS], specifying the
%                     number of rows and columns in the montage. Use NaNs
%                     or Infs to have MONTAGE calculate the size in a
%                     particular dimension in a way that includes all the
%                     images in the montage. For example, if 'Size' is [2
%                     NaN], MONTAGE creates a montage with 2 rows and the
...
%                     between Size and number of images/frames, the montage
%                     is created based on the Size.
%
%                     Default: MONTAGE calculates the rows and columns so
%                     the images in the montage roughly form a square.
%
%   'ThumbnailSize'   A 2-element vector, [TROWS TCOLS], specifying the
%                     size of each individual thumbnail in pixels. The
%                     aspect ratio of the original image will be maintained
%                     by zero-padding the boundary. If one element is NaN
%                     or Inf, the corresponding value is computed
%                     automatically to preserve the aspect ratio of the
%                     first image. If both the elements are NaN or Inf, the
%                     default value is used. ThumbnailSize may be empty
%                     ([]), in which case the full size of the first image
%                     is used as the thumbnail size.
%
%                     Default: MONTAGE automatically computes a
%                     ThumbnailSize based on screen size and number of
%                     images.
%
%   'Parent'          Handle of an axes that specifies the parent of the 
%                     image object created by MONTAGE. The final montage
%                     image is resized to fit in the extents available in
%                     theparent axes.
%
%   'Indices'         A numeric array that specifies which frames MONTAGE
%                     includes in the montage. MONTAGE interprets the
%                     values as indices into array I or cell array
%                     FILENAMES.  For example, to create a montage of the
%                     first four frames in I, use this syntax:
%
...
%                     range of the images in the image array. All images
%                     must be grayscale images. The value LOW (and any
%                     value less than LOW) displays as black, the value
%                     HIGH (and any value greater than HIGH) displays as
%                     white. If you specify an empty matrix ([]), MONTAGE
%                     uses the minimum and maximum values of the images to
%                     be displayed in the montage as specified by
%                     'Indices'. For example, if 'Indices' is 1:K and the
%                     'Display Range' is set to [], MONTAGE displays the
%                     minimum value in of the image array (min(I(:)) as
%                     black, and displays the maximum value (max(I(:)) as
%                     white.
%
...
%                     Default: [0 0]
%
%   'Interpolation'   Interpolation technique when scaling the image to be
%                     displayed, specified as one of the following values:
%
...
%
%   See also IMAGEBROWSER, IMMOVIE, IMSHOW, IMPLAY, IMTILE.
...
function [I,cmap,montageSize,displayRange,displayRangeSpecified,parent,...
    idxs, thumbnailSize, borderSize, backgroundColor,interpolation] = parse_inputs(varargin)
...
paramStrings = {'Size', 'Indices', 'DisplayRange','Parent',...
    'ThumbnailSize', 'BorderSize', 'BackgroundColor','Interpolation'};
